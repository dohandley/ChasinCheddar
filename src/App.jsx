...
import { Upload, ChevronDown, ChevronRight, X, RotateCcw, Check, Minus } from "lucide-react";

// ═══════════════════════════════════════════════════════════════════════════
//  CHASIN CHEDDAR'S COMMAND CENTER v2
//  NBA + MLB · 3 Models per sport: MKB · SimpleOdds · Blended · Performance
// ═══════════════════════════════════════════════════════════════════════════
...
const fmtDate = (d, s) => { try { const t = new Date(d+"T12:00:00"); return s ? t.toLocaleDateString("en-US",{month:"short",day:"numeric"}) : t.toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"}); } catch { return d; } };
...
const calcUnits = picks => {
  let u = 0;
  picks.forEach(p => {
    if (p.hit===true) u += p.best_odds>0 ? p.best_odds/100 : 100/Math.abs(p.best_odds||100);
...
function detectSport(p) {
  if (p.sport) return p.sport.toLowerCase();
  return "mlb";
}
...
export default function App() {
  ...
  useEffect(() => {
    try { const raw = localStorage.getItem(LS); if (raw) setPicks(JSON.parse(raw)); } catch {}
  }, []);
  ...
  const doImport = raw => {
    try {
      ...
      data.forEach(p => {
        ...
        const id = `${p.date||todayStr()}-${sp}-${source}-${nk(p.player)}-${nk(p.market)}-${p.line||0}`;

        const pick = {
          id, date: p.date||todayStr(), source, sport: sp,
      ...
      flash(parts.length ? `Imported: ${parts.join(", ")}` : "No new picks");
      setShowImport(false);
  ...
  const clearSource = (src, sp) => { if(confirm(`Clear all ${SOURCES[src].label} ${sp.toUpperCase()} picks?`)) setPicks(ps => ps.filter(p => !(p.source===src && p.sport===sp))); };
  const clearSport = sp => { if(confirm(`Clear all ${sp.toUpperCase()} picks?`)) setPicks(ps => ps.filter(p => p.sport!==sp)); };
  const clearAll = () => { if(confirm("Clear ALL picks?")) setPicks([]); };

  // Filtered picks
  const sportPicks = picks.filter(p => p.sport===sport);
  const srcPicks = s => sportPicks.filter(p => p.source===s);
  const srcStats = s => {
    const sp = srcPicks(s);
  ...
  const sportCounts = {};
  Object.keys(SPORTS).forEach(sp => { sportCounts[sp] = picks.filter(p=>p.sport===sp).length; });
  ...
  return (
    <div style={{ minHeight:"100vh" }}>
      ...
      <header style={{ background:"linear-gradient(135deg, #0f0f1a, #1a1a2e)", borderBottom:"1px solid rgba(255,255,255,0.06)", padding:"14px 16px" }}>
        <div style={{ maxWidth:960, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <img src="/logo.jpg" alt="Chasin Cheddar" style={{ width:42, height:42, borderRadius:8, objectFit:"cover", border:"1px solid rgba(255,255,255,0.08)" }} />
            <div>
      ...
      {/* SPORT SELECTOR */}
      <div style={{ background:"#0d0d16", borderBottom:"1px solid rgba(255,255,255,0.06)", padding:"0 16px" }}>
        <div style={{ maxWidth:960, margin:"0 auto", display:"flex", gap:0 }}>
          {Object.entries(SPORTS).map(([k, sp]) => (
            <button key={k} onClick={()=>{setSport(k); setTab("mkb"); setSub("picks");}}
              style={{ padding:"10px 20px", background:"none", border:"none", cursor:"pointer",
                color: sport===k?"#e2e8f0":"#4a5568", fontWeight: sport===k?700:500, fontSize:13,
                borderBottom: sport===k?"2px solid #f59e0b":"2px solid transparent" }}>
              {sp.emoji} {sp.label}
              {sportCounts[k]>0 && <span style={{ marginLeft:6, fontSize:10, background:"rgba(255,255,255,0.08)", padding:"1px 6px", borderRadius:8, color:"#94a3b8" }}>{sportCounts[k]}</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <nav style={{ background:"#0d0d16", borderBottom:"1px solid rgba(255,255,255,0.06)", padding:"0 16px", overflowX:"auto" }}>
        <div style={{ maxWidth:960, margin:"0 auto", display:"flex", gap:0 }}>
          {[...Object.entries(SOURCES), ["performance", { label:"Performance", emoji:"📊", color:"#a78bfa" }]].map(([k, src]) => {
            const count = k!=="performance" ? sportPicks.filter(p=>p.source===k).length : null;
            return (
              <button key={k} onClick={()=>{setTab(k); setSub(k==="performance"?"overview":"picks");}}
                style={{ flex:1, minWidth:0, padding:"10px 6px", background:"none", border:"none", cursor:"pointer",
      ...
      <main style={{ maxWidth:960, margin:"0 auto", padding:"14px 16px 100px" }}>
        {tab==="performance" ? (
          <PerformanceView sportPicks={sportPicks} srcStats={srcStats} sport={sport} />
        ) : sub==="picks" ? (
          <PicksView picks={srcPicks(tab)} source={tab} sport={sport} resolve={resolve} unresolve={unresolve} />
        ) : sub==="history" ? (
          <HistoryView picks={srcPicks(tab)} source={tab} resolve={resolve} unresolve={unresolve} />
        ) : (
          <StatsView picks={srcPicks(tab)} source={tab} sport={sport} />
        )}
      </main>

      {showImport && <ImportModal onImport={doImport} onClose={()=>setShowImport(false)} clearSource={clearSource} clearSport={clearSport} clearAll={clearAll} picks={picks} sport={sport} />}
...
function PicksView({ picks, source, sport, resolve, unresolve }) {
  ...
  if (!day.length) return <Empty emoji={src.emoji} text={`No ${src.label} ${sp.label} picks yet`} sub="Import your JSON to get started" />;
  ...
  return (
    <div>
      ...
      {bb.length>0 && <PickTable title={`${sp.emoji} Best Bets`} sub={`${bb.length} picks · ${fmtDate(showDate,true)}`} picks={bb} accent={src.color} resolve={resolve} unresolve={unresolve} />}
...
function DayAccordion({ date, picks, hits, decided, units, resolve, unresolve }) {
  ...
  return (
    <div style={{ background:"rgba(255,255,255,0.02)", borderRadius:10, overflow:"hidden" }}>
      ...
      {open && <div style={{ borderTop:"1px solid rgba(255,255,255,0.04)" }}>{picks.sort((a,b)=>b.score-a.score).map((p,i) => <PickRow key={p.id} p={p} rank={i+1} resolve={resolve} unresolve={unresolve} />)}</div>}
    </div>
  );
}
...
function StatsView({ picks, source, sport }) {
  const src = SOURCES[source];
  const sp = SPORTS[sport];
  const res = picks.filter(p=>p.resolved&&(p.hit===true||p.hit===false));
  if (!res.length) return <Empty emoji={src.emoji} text={`No resolved ${src.label} ${sp.label} picks`} sub="Resolve picks or import tracker results" />;
  ...
  return (
    <div>
      ...
      <Card title={`${sp.emoji} By Section`}>
        <Table headers={["Section","Decided","Hit Rate","Units"]} rows={[
          [`${sp.emoji} Best Bets`, bbRes.length, bbRes.length?`${Math.round(bbRes.filter(p=>p.hit===true).length/bbRes.length*100)}%`:"—", fmtU(calcUnits(bbRes))],
          ["🎰 Lotto", lotRes.length, lotRes.length?`${Math.round(lotRes.filter(p=>p.hit===true).length/lotRes.length*100)}%`:"—", fmtU(calcUnits(lotRes))],
        ]} />
      </Card>
      <Card title="By Grade">
        <Table headers={["Grade","Decided","Hit Rate","Units"]} rows={Object.entries(byGrade).map(([g,d]) => [`${GRADE[g].emoji} ${g}`, d.d, `${Math.round(d.h/d.d*100)}%`, fmtU(d.u)])} />
      </Card>
...
function PerformanceView({ sportPicks, srcStats, sport }) {
  const sp = SPORTS[sport];
  ...
  if (!hasData) return <Empty emoji="📊" text={`No resolved ${sp.label} picks yet`} sub="Import data and resolve picks to compare models" />;
  ...
  return (
    <div>
      ...
      <Card title={`🏷️ ${sp.emoji} Head-to-Head`}>
...
function ImportModal({ onImport, onClose, clearSource, clearSport, clearAll, picks, sport }) {
    ...
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100, padding:16 }}>
      <div style={{ background:"#1a1f2e", borderRadius:16, padding:20, maxWidth:480, width:"100%", maxHeight:"85vh", overflow:"auto", border:"1px solid rgba(255,255,255,0.06)" }}>
        ...
        <p style={{ fontSize:11, color:"#64748b", margin:"0 0 14px", lineHeight:1.5 }}>
          Drop or paste any picks JSON. Source and sport auto-detected.
        </p>
          ...
          <div style={{ marginTop:14, paddingTop:12, borderTop:"1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {Object.entries(SOURCES).map(([k,v]) => {
                const n = sportPicks.filter(p=>p.source===k).length;
                return n>0 ? <button key={k} onClick={()=>clearSource(k, sport)} style={S.dangerBtn}>{v.emoji} Clear {v.label} ({n})</button> : null;
              })}
              {sportPicks.length>0 && <button onClick={()=>clearSport(sport)} style={S.dangerBtn}>Clear All {sport.toUpperCase()} ({sportPicks.length})</button>}
              <button onClick={clearAll} style={S.dangerBtn}>Clear Everything ({picks.length})</button>
            </div>
          </div>
        )}
      </div>
...
function PickRow({ p, rank, resolve, unresolve }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ width:24, textAlign:"center", fontWeight:700, color:rank<=3?"#f59e0b":"#4a5568", flexShrink:0 }}>
        {rank<=3?["🥇","🥈","🥉"][rank-1]:rank}
      </div>
      ...
      <div style={{ flexShrink:0 }}>
        <span style={{ fontSize:10, fontWeight:600, padding:"2px 6px", borderRadius:4,
          background:isU?"rgba(239,68,68,0.08)":"rgba(34,197,94,0.08)", color:isU?"#ef4444":"#22c55e" }}>
      ...
      <div style={{ width:68, textAlign:"center", flexShrink:0 }}>
        {p.resolved ? (
          <div style={{ display:"flex", alignItems:"center", gap:3, justifyContent:"center" }}>
            ...
            <button onClick={()=>unresolve(p.id)} style={{ background:"none", border:"none", cursor:"pointer", color:"#4a5568", padding:0 }}><RotateCcw size={10}/></button>
          </div>
        ) : (
...
function Table({ headers, rows }) {
  if (!rows.length) return <div style={{ textAlign:"center", padding:16, color:"#4a5568", fontSize:12 }}>No data</div>;
  return (
    <div style={{ overflowX:"auto" }}>
      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
        <thead><tr>{headers.map((h,i) => <th key={i} style={{ padding:"5px 6px", textAlign:i===0?"left":"center", color:"#4a5568", fontWeight:600, fontSize:9, textTransform:"uppercase", letterSpacing:"0.05em", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>{h}</th>)}</tr></thead>
        <tbody>{rows.map((row,i) => <tr key={i}>{row.map((cell,j) => {
          ...
          return <td key={j} style={{ padding:"6px", textAlign:j===0?"left":"center", fontWeight:j===0?600:isU?700:400,
            fontFamily:isU?"'JetBrains Mono',monospace":"inherit", color:pos?"#22c55e":neg?"#ef4444":"#e2e8f0",
            borderBottom:"1px solid rgba(255,255,255,0.025)" }}>{cell}</td>;
        })}</tr>)}</tbody>
      </table>
    </div>
  );
